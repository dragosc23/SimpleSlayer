import pygame
from src.game_objects.item_definitions import HealthPotion # Modified import
# Removed local RARITY_COLORS definition and specific rarity imports from src.items
from src.utils.constants import WHITE, LIGHT_BLUE, LIGHT_GRAY, DARKER_GRAY, RARITY_COLORS # Added import
...
def draw_player_health(screen, player_health, player_max_health):
    ...
    text_surface = HEALTH_FONT.render(f"Health: {player_health}/{player_max_health}", True, WHITE) # Modified
...
def draw_wave_info(screen, wave_number, total_waves, enemies_remaining, game_state): # Add game_state
    ...
    text_surface = INFO_FONT.render(wave_text, True, WHITE) # Modified
...
def draw_player_stats(screen, player):
    ...
    text_surface_base_atk = INFO_FONT.render(base_atk_text, True, WHITE) # Modified
    ...
    text_surface_total_atk = INFO_FONT.render(total_atk_text, True, WHITE) # Modified
...
def draw_player_experience(screen, player):
    ...
    level_surface = INFO_FONT.render(level_text, True, WHITE) # Modified
    xp_surface = INFO_FONT.render(xp_text, True, WHITE) # Modified
...
def draw_inventory_preview(screen, player_inventory):
    ...
    capacity_surface = INFO_FONT.render(capacity_text, True, WHITE) # Modified
    ...
    potion_count_surface = INFO_FONT.render(potion_count_text, True, LIGHT_BLUE) # Modified
    ...
    for i, item in enumerate(player_inventory.items):
        ...
        item_color = RARITY_COLORS.get(item_rarity, RARITY_COLORS["Default"]) # Uses imported RARITY_COLORS
            
        item_text_display = f"- {str(item)}"

        item_surface = INFO_FONT.render(item_text_display, True, item_color) # Modified
    ...
    if len(player_inventory.items) > items_shown_count:
        ...
        more_items_surface = INFO_FONT.render(more_items_text, True, DARKER_GRAY) # Modified
